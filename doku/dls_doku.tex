...
\item[\texttt{dls} (Init-Script)]\index{rcdls@\texttt{dls} (Init-Script)}
  Zum Starten und Stoppen des \textit{dlsd} und aller damit
  zusammenhängenden Dienste gibt es ein Init-Script\index{Init-Script}
  (oder RC-Script) \texttt{dls}, das standardmäßig in
  \textit{/etc/init.d} installiert wird. Zusätzlich wird ein Soft-Link
  ...
  \textit{\$DLS\_DIR}\index{\$DLS\_DIR@\textit{\$DLS\_DIR}} und
...
\item[\texttt{dls\_status}]\index{dls\_status@\texttt{dls\_status}
  (Script)} Zur allgemeinen Überwachung der Datenerfassung gibt es das
  Script \texttt{dls\_status}. Es ist ein Kommandozeilentool, das die
...
\item Das betätigen der Schalfläche \glqq Aktualisieren\grqq\ liest
  ...
  an.
\item \textit{Die Schaltfläche \glqq Exportieren\grqq\ ist momentan
    nicht benutzbar}
...
(\textit{http://www.gzip.org/zlib}) stellt Funktionen zur
...
\item Für das Kompilieren der grafischen Benutzeroberflächen
  ...
  \begin{quote}
    \textit{http://www.fltk.org}
  \end{quote}
  heruntergeladen werden kann.
\item Für die Kompression wird die \textit{ZLib} benötigt. Diese ist in nahezu
  jeder Linux-Distribution enthalten. Ansonsten kann sie von der
  \textit{ZLib}-Homepage
  \begin{quote}
    \textit{http://www.gzip.org/zlib}
  \end{quote}
...
\item Ebenfalls für die Kompression wird die \textit{FFTW}-Bibliothek
  ...
  Homepage:
  \begin{quote}
    \textit{http://www.fftw.org/download.html}
  \end{quote}
  heruntergeladen werden.
...
\index{rcdls@\texttt{rcdls} (Script)!Kommandozeilenparameter}
...
\index{dls\_status@\texttt{dls\_status} (Script)!Kommandozeilenparameter}
