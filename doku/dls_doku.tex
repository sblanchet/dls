...
\begin{titlepage}
  \begin{center}
    {\large Ingenieurgemeinschaft IgH}
    \vfill
    {\Large Dokumentation:}\\
    \vspace{5mm}
    {\Huge Data Logging Server (DLS)}
    \vfill
    {\Large Florian Pose}
    \vfill
    Der \glqq\textsl{DLS}\grqq\ ist ein Messdaten-Erfassungssystem,
...
\glqq\textsl{DLS}\grqq)\index{DLS@\textsl{DLS}} ist ein
...
\glqq\textsl{Datenquelle}\grqq, \index{Datenquelle!Definition} die
Messdaten liefert. In diesem Fall ist dies ein Server, der die
Messdaten der von der \textsl{IgH} entwickelten \textsl{rt\_lib} über
das Netzwerk bereitstellt. Die Kommunikation mit der Datenquelle wird
in Kapitel \ref{sec:dlsd_logger_comm} beschrieben.

Alle zu liefernden Daten sind in
\glqq\textsl{Kanälen}\grqq\index{Kanal!Definition} organisiert. Ein
Kanal ist die Abstraktion einer messbaren, physikalischen Größe, die
von der Datenquelle angeboten wird. Eigenschaften eines Kanales sind
die Einheit, \index{Kanal!Einheit} die maximale Abtastfrequenz
...
Der \textsl{DLS} erfasst Daten über
sog.
\glqq\textsl{Messaufträge}\grqq\index{Messauftrag!Definition}. Diese
beinhalten allgemeine Vorgaben zur Erfassung zusammen mit der Liste
der zu erfassenden Kanäle und deren Vorgaben. Ein Messauftrag ist zwar
immer an eine bestimmte Datenquelle gebunden. Es können beliebig viele
...
erfassenden Kanal eine sog. \glqq\textsl{Kanalvorgabe}\grqq
\index{Kanalvorgabe} existieren. Diese fasst die Bedingungen zusammen,
unter denen Daten von einem Kanal erfasst und gespeichert werden
sollen. Dies sind die Abtastrate, die Blockgröße, die zu erfassenden
Meta-Daten (siehe Kapitel \ref{sec:dlsd_data_meta}), die
...
Kanalinformationen nach Messaufträgen sortiert im \glqq
\textsl{DLS}-Datenverzeichnis\grqq
\index{DLS-Datenverzeichnis@\textsl{DLS}-Datenverzeichnis}
...
\textsl{DLS}-Packetes entweder über Kommandozeilenparameter (Option
\texttt{-d}) mitgeteilt, oder in der Umgebungsvariable
\texttt{\$DLS\_DIR}\index{\$DLS\_DIR@\texttt{\$DLS\_DIR}} abgelegt
werden. Die Suchreihenfolge ist immer: Parameter - Umgebungsvariable -
aktuelles Verzeichnis.

Es können durchaus mehrere \textsl{DLS}-Datenverzeichnisse existieren,
die aber von unterschiedlichen Instanzen des \textsl{DLS}-Daemons
bedient werden müssen (siehe Kapitel \ref{sec:dlsd_mother}).

Eine genaue Beschreibung der Struktur des
\textsl{DLS}-Datenverzeichnisses und der darin enthaltene Daten
...
\item \texttt{rcdls} \index{rcdls@\texttt{rcdls} (Script)}\\
  Zum Starten und Stoppen des \textsl{dlsd} und aller damit
  zusammenhängenden Dienste gibt es ein Init-Script\index{Init-Script}
  (oder RC-Script) \texttt{dls}, das standardmäßig in
  \texttt{/etc/init.d} installiert wird. Zusätzlich wird ein Soft-Link
  \texttt{/usr/local/bin/rcdls} angelegt, der auf das Init-Script
  zeigt. Dieses kennt die Parameter \texttt{start}, \texttt{stop},
  \texttt{restart} und \texttt{status}. Die Konfiguration des Dienstes
  erfolg über eine Sysconfig-Datei\index{Sysconfig-Datei}
  mit Namen \texttt{/etc/sysconfig/dls}, die alle nötigen Variablen
  enthält. Es wird empfohlen, die enthaltenen Variablen
  \texttt{\$DLS\_DIR}\index{\$DLS\_DIR@\texttt{\$DLS\_DIR}} und
  \texttt{\$DLS\_USER}\index{\$DLS\_USER@\texttt{\$DLS\_USER}} für
  alle Benutzer zu exportieren und so allen Werkzeugen des
  \textsl{DLS}-Packetes zugänglich zu machen.

\item \texttt{dls\_status}\index{dls\_status@\texttt{dls\_status} (Script)}\\
  Zur allgemeinen Überwachung der Datenerfassung gibt es das Script
  \texttt{dls\_status}. Es ist ein Kommandozeilentool, das die
  grundsätzliche Bereitschaft zur Messdatenerfassung für ein bestimmtes
  \textsl{DLS}-Datenverzeichnis anzeigen kann. Es gibt aus, ob der
  \textsl{DLS}-Mutterprozess läuft, welche Messaufträge es gibt, und
  ob für diese die jeweiligen \textsl{DLS}-Erfassungsprozesse laufen.

\end{itemize}

%------------------------------------------------------------------------------

\chapter{Der \textsl{DLS}-Daemon (\textsl{dlsd})}
\label{sec:dlsd}
...
\index{DLS-Datenverzeichnis@\textsl{DLS}-Datenverzeichnis} gebunden
...
\section{Der \textsl{dlsd}-Mutterprozess}
...
\textsl{DLS}-Datenverzeichnis
\index{DLS-Datenverzeichnis@\textsl{DLS}-Datenverzeichnis}
arbeiten. Es sind Schutzmechanismen implementiert, die dieses
gewährleisten (\textsl{PID}-Dateien, siehe Anhang
...
\textsl{DLS}-Datenverzeichnis ein und spaltet für jeden aktiven
Messauftrag\index{Messauftrag} eine Kopie seiner selbst
(\textsl{dlsd}-Er\-fass\-ungs\-pro\-zess, siehe Kapitel
...
{\ttfamily spool}. Der \textsl{dlsd}-Mut\-ter\-pro\-zess leert dieses
...
\textsl{dlsd}-Mutterprozess verarbeitet:
...
\item \textsl{SIGSEGV} (und weitere, kritische Signale)\\
  ...
  \textsl{DLS}-Datenverzeichnis hinterlassen, die Informationen über
...
\section{Der \textsl{dlsd}-Erfassungsprozess}
\label{sec:dlsd_logger}

\index{dlsd@\textsl{dlsd}!Erfassungsprozess}

Der vom \textsl{dlsd}-Mutterprozess abgespaltene Erfassungsprozess ist
...
\textsl{dlsd}-Mutterprozess mitgegeben wurde. Dieser Messauftrag wird
über das \textsl{DLS}-Datenverzeichnis und die entsprechende
Messauftrags-ID eindeutig identifiziert. Jegliche Daten zu diesem
Auftrag werden dort im Unterverzeichnis \texttt{job\textsl{<ID>}}
abgelegt (siehe Kapitel \ref{sec:data}).
...
Der \textsl{dlsd}-Erfassungsprozess importiert zunächst die Vorgaben
seines Messauftrags \index{Messauftrag} aus der zentralen
Vorgabendatei \texttt{job\textsl{<ID>}/job.xml} und verbindet sich
dann über TCP/IP mit der dort angegebenen Datenquelle (Beschreibung
des Kommunikationsprotokolles, siehe Kapitel
...
\item \texttt{index} - Die Position des Kanals innerhalb der
  ...
  Kanalverzeichnis innerhalb des \textsl{DLS}-Datenverzeichnis
...
\item \texttt{bufsize} - Größe des Ringspeichers innerhalb der
  ...
  $ \hbox{BlockSize} * \hbox{Reduction} \stackrel{!}{\le}
...
Folgende Signale \index{Signalbehandlung} werden vom
\textsl{dlsd}-Erfassungsprozess verarbeitet:
...
\chapter{Das \textsl{DLS}-Datenverzeichnis}
\label{sec:data}
...
\textsl{DLS}-Daten\-ver\-zeich\-nis\-ses. Hier liegen hauptsächlich
Dateien für den \textsl{dlsd}-Mutterprozess. Darüberhinaus liegen im
...
dem Parameter \texttt{-d} das \textsl{DLS}-Daten\-verzeich\-nis
...
\chapter{Der Betrachter \textsl{DLS View}}
\label{sec:view}
