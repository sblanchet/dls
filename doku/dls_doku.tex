...
\textit{DLS})\index{DLS@\textit{DLS}} ist ein
Messdatenerfassungssystem, das in der Lage ist, beliebige Messdaten
über lange Zeit zu erfassen, komprimieren, archivieren und bei Bedarf
auch schnell wieder auszugeben.
...
Einheit, \index{Kanal!Einheit} die maximale Abtastfrequenz
\index{Kanal!Abtastfrequenz} und der Datentyp. \index{Kanal!Datentyp}

Der \textit{DLS} kann sich mit der Datenquelle verbinden und so
...
Der \textit{DLS} erfasst Daten über sog.
...
\textit{DLS}-Datenverzeichnis
\index{DLS-Datenverzeichnis@\textit{DLS}-Datenverzeichnis}
gespeichert. Dieses ist standardmäßig das aktuelle Verzeichnis. Wenn
ein anderer Ablageort gewünscht ist, kann dies den Programmen des
\textit{DLS}-Packetes entweder über Kommandozeilenparameter (Option
\textit{-d}) mitgeteilt, oder in der Umgebungsvariable
\textit{\$DLS\_DIR}\index{\$DLS\_DIR@\textit{\$DLS\_DIR}} abgelegt
werden. Die Suchreihenfolge ist immer: Parameter - Umgebungsvariable -
aktuelles Verzeichnis.

Es können durchaus mehrere \textit{DLS}-Datenverzeichnisse existieren,
die aber von unterschiedlichen Instanzen des \textit{DLS}-Daemons
bedient werden müssen (siehe Kapitel \ref{sec:dlsd_mother}).

Eine genaue Beschreibung der Struktur des
\textit{DLS}-Datenverzeichnisses und der darin enthaltene Daten
befindet sich in Kapitel \ref{sec:data}.
...
\begin{description}
\item[DLS Manager] \index{DLS Manager@\textit{DLS Manager}}
  Messaufträge \index{Messauftrag} können über eine grafische
  Benutzeroberfläche, den \textit{DLS Manager} (siehe Kapitel
...
\item[DLS View] \index{DLS View@\textit{DLS View}} Zur Ansicht der
  Daten existiert ebenfalls ein grafisches Tool \textit{DLS View}
  (siehe Kapitel \ref{sec:view}). Der Benutzer kann damit beliebige,
  erfasste Kanäle eines Messauftrags über einer gemeinsamen Zeitskala
  anzeigen. Dabei ist auch die Navigation im Zeitfenster und das
  Bestimmen einzelner Datenwerte möglich.
\item[dls] Das Kommandozeilenwerkzeug
  \textit{dls}\index{dls@\textit{dls} (Tool)} kennt Befehle um bereits
  erfasste Daten einzusehen und zu exportieren. Siehe
  Abschnitt~\ref{sec:apx_cmd_dls} für eine Auflistung.
\item[Init-Script]\index{Init-Script} Zum Starten und Stoppen des
  \textit{dlsd} und aller damit zusammenhängenden Dienste gibt es ein
  Init-Script, das standardmäßig nach \textit{/etc/init.d/dls}
  installiert wird. Zusätzlich wird ein Soft-Link
  \textit{/usr/local/bin/rcdls} angelegt, der auf das Init-Script
  zeigt. Dieses kennt die Parameter \textit{start}, \textit{stop},
  \textit{restart} und \textit{status}. Die Konfiguration des Dienstes
  erfolg über eine Sysconfig-Datei\index{Sysconfig-Datei} mit Namen
  \textit{/etc/sysconfig/dls}, die alle nötigen Variablen enthält. Es
  wird empfohlen, die enthaltenen Variablen
  \textit{\$DLS\_DIR}\index{\$DLS\_DIR@\textit{\$DLS\_DIR}},
  \textit{\$DLS\_EXPORT}\index{\$DLS\_EXPORT@\textit{\$DLS\_EXPORT}},
  \textit{\$DLS\_EXPORT\_FMT}
  \index{\$DLS\_EXPORT\_FMT@\textit{\$DLS\_EXPORT\_FMT}} und
  \textit{\$DLS\_USER}\index{\$DLS\_USER@\textit{\$DLS\_USER}} für
  alle Benutzer zu exportieren und so allen Werkzeugen des
  \textit{DLS}-Packetes zugänglich zu machen.
\item[dls\_status]\index{dls\_status@\textit{dls\_status}} Zur
  allgemeinen Überwachung der Datenerfassung gibt es das Script
  \textit{dls\_status}. Es ist ein Kommandozeilentool, das die
  grundsätzliche Bereitschaft zur Messdatenerfassung für ein
  bestimmtes \textit{DLS}-Datenverzeichnis anzeigen kann. Es gibt aus,
  ob der \textit{DLS}-Mutterprozess läuft, welche Messaufträge es
  gibt, und ob für diese die jeweiligen
  \textit{DLS}-Erfassungsprozesse laufen.
\end{description}

%------------------------------------------------------------------------------

\chapter{Der \textit{DLS}-Daemon (\textit{dlsd})}
\label{sec:dlsd}

Für die gesamte Datenerfassung und -ablage ist der \textit{DLS-Daemon}
(kurz: \textit{dlsd}) \index{dlsd@\textit{dlsd}} zuständig. Er ist
ein Prozess, der im Hintergrund (ohne Verbindung zu einer Konsole)
arbeitet und an ein bestimmtes \textit{DLS}-Datenverzeichnis
\index{DLS-Datenverzeichnis@\textit{DLS}-Datenverzeichnis} gebunden
...
\section{Der \textit{dlsd}-Mutterprozess}
\label{sec:dlsd_mother}

\index{dlsd@\textit{dlsd}!Mutterprozess}

Ein mit dem Befehl \textit{dls start} gestarteter
\textit{dlsd}-Prozess übernimmt die Überwachung der
Messaufträge. \index{Messauftrag} Er heisst dann
\textit{dlsd}-Mutterprozess. Er hält im Speicher eine Liste von
eigenen Kopien der im \textit{DLS}-Datenverzeichnis vorhandenen
...
\textit{DLS}-Datenverzeichnis
\index{DLS-Datenverzeichnis@\textit{DLS}-Datenverzeichnis}
arbeiten. Es sind Schutzmechanismen implementiert, die dieses
gewährleisten (\textit{PID}-Dateien, siehe Anhang
...
\textit{DLS}-Datenverzeichnis ein und spaltet für jeden aktiven
...
\item Prüfen, ob zwischenzeitlich Signale empfangen wurden. Das kann
  beispielsweise passieren, wenn der \textit{dlsd} beendet werden soll,
...
den Benutzer nicht auch gleichzeitig vom \textit{dlsd} gelesen
würde. Dadurch könnten Fehler in der Verarbeitung entstehen.

Im \textit{DLS}-Datenverzeichnis existiert daher ein Unterverzeichnis
{\ttfamily spool}. Der \textit{dlsd}-Mut\-ter\-pro\-zess leert dieses
...
Ein gültiges Spooling-Kommando ist für den \textit{dlsd} eine Datei
...
\item Kennt er einen Auftrag mit dieser ID, aber die Vorgabendatei
  existiert \textbf{nicht}, geht der \textit{dlsd} davon aus, dass der
...
\textit{dlsd}-Mutterprozess verarbeitet:
...
\item[SIGCHLD] Ein Kindprozess wurde beendet. Dies kann
  ...
  \item Der Prozess hat einen internen Fehler festgestellt und sich
    ...
    Warnung, dass die Erfassung nicht mehr läuft. Es wird lediglich eine
    Nachricht an den \textit{syslogd} \index{syslogd@\textit{syslogd}}
    verschickt.
  ...
  \end{itemize}
\item[SIGINT/SIGTERM] Der \textit{dlsd} soll beendet werden. Der
  Mutterprozess wird das Signal daraufhin an alle seine Kindprozesse
  weiterleiten, warten bis diese ihre Daten gespeichert haben und sich
...
\item[SIGSEGV u. A.] Diese Signale werden zur Sicherheit überwacht und
  vom \textit{dlsd}-Mutterprozess und -Erfassungsprozess gleichartig
  ...
  \textit{DLS}-Datenverzeichnis hinterlassen, die Informationen über
  das empfangene Signal enthält, und sich kurz darauf mit dem
  Rückgabewert $-3$ beenden.
\end{description}

%------------------------------------------------------------------------------

\section{Der \textit{dlsd}-Erfassungsprozess}
\label{sec:dlsd_logger}

\index{dlsd@\textit{dlsd}!Erfassungsprozess}

Der vom \textit{dlsd}-Mutterprozess abgespaltene Erfassungsprozess ist
für die Kommunikation mit der Datenquelle, das Komprimieren der
empfangenen Daten und das Speichern der komprimierten Daten auf der
Festplatte zuständig. Er ist einem bestimmten Messauftrag
\index{Messauftrag} zugeordnet, der ihm beim Start vom
\textit{dlsd}-Mutterprozess mitgegeben wurde. Dieser Messauftrag wird
über das \textit{DLS}-Datenverzeichnis und die entsprechende
Messauftrags-ID eindeutig identifiziert. Jegliche Daten zu diesem
Auftrag werden dort im Unterverzeichnis \textit{job\textless
...
Der \textit{dlsd}-Erfassungsprozess importiert zunächst die Vorgaben
seines Messauftrags \index{Messauftrag} aus der zentralen
Vorgabendatei \textit{job\textless ID\textgreater/job.xml} und
verbindet sich dann über TCP/IP mit der dort angegebenen Datenquelle
(Beschreibung des Kommunikationsprotokolles, siehe Kapitel
\ref{sec:dlsd_logger_comm}).

Die erfassten Messdaten laufen für jeden Kanal in einen
sog. \textit{Block-Buffer}. Ist dieser voll, werden die Daten
...
\textit{Block-Buffers} kann vom Benutzer in den Kanalvorgaben
angegeben werden.
...
können, speichert der \textit{DLS}-Erfassungsprozess nicht nur die
...
\textit{dlsd}-Erfassungsprozess parallel zum \textit{Block-Buffer} ein
sog. \textit{Meta-Buffer}. Dieser Speicher fasst $u$ Datenwerte, wobei $u$ die
...
der \textit{Meta-Buffer} voll, so wird aus den $u$ generischen Datenwerten ein
\glqq Meta-Datenwert\grqq\ der Meta-Ebene 1 generiert und dort wiederum in
einem \textit{Block-} und einem \textit{Meta-Buffer} abgelegt. Für diese
...
die auch gleichzeitig erzeugt werden kön\-nen. Momentan werden Folgende
unterstützt:
...
bitweise \textit{ODER}-Verknüpfung der angegebenen
...
würde. Die überschüssigen Werte in den \textit{Meta-Buffern} werden also
verworfen.
...
\index{XML} angelehnte Protokoll der \textit{rt\_lib} (Version $\ge 2.7$) der
\textit{IgH}. Die Verbindung zur Datenquelle erfolgt über TCP/IP (Port 2345).
...
Kompatiblität mit der aktuellen \textit{dlsd}-Version
geprüft. Zusätzlich kann das \textit{\textless
connected\textgreater}-Tag ein Attribut \textit{arch} enthalten, das
die Architektur (\glqq Endianess\grqq) \index{Endianess} der
Datenquelle und somit der gesendeten Binärdaten enthält. Mögliche
Werte sind \textit{big} (für \glqq big-endian\grqq) oder
\textit{little} (für \glqq little-endian\grqq). Existiert kein
\textit{arch}-Attribut, wird als Quellenarchitektur \glqq
little-endian\grqq\ angenommen und eine Warnung ausgegeben.

\paragraph{Bestimmung der maximalen Abtastfrequenz}
Nachdem das \textit{\textless connected\textgreater }-Tag empfangen
und überprüft wurde, fragt der Erfassungsprozess zunächst den
...
\item[index] - Die Position des Kanals innerhalb der
  Liste. Diese wird später auch als Identifier für ein
  Kanalverzeichnis innerhalb des \textit{DLS}-Datenverzeichnis
  verwendet (siehe Kapitel \ref{sec:data}).
...
Der \textit{dlsd} kennt Mechanismen zum Begrenzen des erforderlichen
...
\textit{DLS}-Quota-Daemon. Dieser muss immer parallel zum
\textit{dlsd} laufen, sobald in mindestens einem Auftrag Quotas
konfiguriert sind. Der Start erfolgt manuell mit
\textit{dls\_quota}\index{dls\_quota@\textit{dls\_quota}}.
Kommandozeilenparameter sind in Anhang \ref{sec:apx_cmd_quota}
einsehbar.
...
Da der \textit{DLS}-Quota-Daemon immer nur komplette Chunks \index{Chunk} auf
...
existieren, auch wenn der \textit{dlsd}-Erfassungsprozess nicht unterbrochen
wurde.

Daher überwacht der \textit{dlsd}-Erfassungsprozess die
...
\textit{dlsd}-Erfassungsprozesses vereinbar ist, wird zum Speichern
der restlichen, erfassten Daten ein eigener Prozess erzeugt. Dieser
\glqq Aufräumprozess\grqq\index{Aufräumprozess} speichert nun alle
...
\textit{time} enthalten, dass den Zeitstempel der Nachricht in Sekunden
enthält und -je nach Nachricht- ein Attribut \textit{text}, das aber vom
\textit{dlsd} nicht weiter ausgewertet wird.
...
Der \textit{dlsd}-Erfassungsprozess speichert die Nachrichten
im Unterverzeichnis \textit{messages} des Auftragsverzeichnisses (siehe
...
Folgende Signale \index{Signalbehandlung} werden vom
\textit{dlsd}-Erfassungsprozess verarbeitet:
...
\item[SIGCHLD] Ein \glqq Aufräumprozess\grqq\index{Aufräumprozess}
  ...
  wird lediglich über den \textit{syslogd}
  \index{syslogd@\textit{syslogd}} vermerkt.
...
\chapter{Das \textit{DLS}-Datenverzeichnis}
\label{sec:data}

Alle persistenten Daten des \textit{DLS}-Systems sind in
\textit{DLS-Datenverzeichnissen}
\index{DLS-Datenverzeichnis@\textit{DLS}-Datenverzeichnis} organisiert. Die
...
\begin{figure}[htb]
  ...
  \end{center}
  \caption{Struktur des \textit{DLS}-Datenverzeichnisses}
  \label{fig:dls_data}
\end{figure}
...
\textit{DLS}-Daten\-ver\-zeich\-nis\-ses. Hier liegen hauptsächlich
Dateien für den \textit{dlsd}-Mutterprozess. Darüberhinaus liegen im
Wurzelverzeichnis auch alle Auftragsverzeichnisse (siehe Kapitel
\ref{sec:data_jobs}).
...
\item[id\_sequence] Diese Datei enthält die nächste, freie Auftrags-ID
  als \textit{ASCII}-kodierte Ziffernfolge. Sie wird vom \textit{DLS
...
\item[dlsd.pid] Dies ist die \textit{PID}-Datei des
  \textit{dlsd}-Mutterprozesses (siehe Anhang
  \ref{sec:apx_pid}). \index{PID-Dateien} Sie wird zur Laufzeit
  automatisch angelegt und zeigt an, dass gerade ein
  \textit{dlsd}-Mutterprozess läuft.
\item[job\textless ID\textgreater] Jedes Auftragsverzeichnis liegt im
  \textit{DLS}-Wurzelverzeichnis. Der Name ist immer \textit{job},
  gefolgt von der Auftrags-ID. Siehe Kapitel \ref{sec:data_jobs}.
\item[spool] Dies ist das Spooling-Verzeichnis des
  \textit{dlsd}-Mutterprozesses. Eine Beschreibung steht in Kapitel
...
im laufenden Betrieb von einem eigenen
\textit{dlsd}-Erfassungs\-prozess bearbeitet. Dieser liest von dort
seine Vorgaben und schreibt auch die erfassten Daten dorthin.
...
\item[job.xml] Die zentrale Vorgabendatei für einen Auftrag. Sie
  ...
  erzeugt werden, damit dieser die neuen Vorgaben übernimmt. Der
  \textit{DLS Manager} macht dies automatisch.
  ...
  Die Attribute \textit{mdct\_block\_size} bzw. \textit{mdct\_accuracy} werden
  nur benötigt, wenn das Kompressionsformat auf der MDCT (siehe Kapitel
  \ref{sec:comp_mdct}) basiert.

  Die Vorgaben können mit dem \textit{DLS Manager} editiert
...
\item[watchdog und logging] Dies sind zwei leere Dateien, die für die
  Überwachung der \textit{dlsd}-Erfassungprozesse seitens des
  \textit{DLS Managers} verwendet werden. Läuft ein Erfassungsprozess
  für das Auftragsverzeichnis, so ändert er jede Sekunde den
  Zeitstempel der Datei \textit{watchdog}. Erfasst der Prozess
  gleichzeitig auch Daten, so verfährt er ebenso mit der Datei
  \textit{logging}. Der \textit{DLS Manager} prüft die Zeitstempel
  dieser Dateien regelmäßig, erhält so Aufschluß über den Zustand des
  Erfassungsprozesses und kann dies so dem Benutzer anzeigen.
\item[dlsd.pid] Dies ist die \textit{PID}-Datei des
  \textit{dlsd}-Erfassungsprozesses (siehe Anhang
  \ref{sec:apx_pid}). \index{PID-Dateien} Sie wird zur Laufzeit
  automatisch angelegt und zeigt an, dass der Erfassungsprozess gerade
  läuft.
\item[channel\textless INDEX\textgreater] Die erfassten Daten sind
  weiterhin in Kanälen organisiert, die jeweils ihr eigenes
  Kanal-Verzeichnis besitzen (siehe Kapitel
  \ref{sec:data_channels}). Der Index im Namen des Kanalverzeichnisses
  entspricht dem Kanalindex, den der Befehl \textit{\textless
  rk\textgreater} beim Auslesen aller Kanäle der Datenquelle während
  des Startvorganges des \textit{dlsd}-Mutterprozesses zurückgeliefert
  hat (siehe Kapitel \ref{sec:dlsd_logger_comm}).
...
\textit{dlsd}-Erfassungsprozess überprüft, wenn eine neue Erfassung in ein
...
(\textit{chunk\textless ZEITSTEMPEL\textgreater}). Ein Chunk
\index{Chunk} ist eine lückenlos erfasste Serie von Daten, die ab
einem bestimmten Zeitpunkt mit der selben Kanalvorgabe erfasst
wurden. Der Zeitstempel im Verzeichnisnamen ist der Zeitstempel des
ersten Datenwertes im Chunk. Zur Beschreibung der Chunk-Eigenschaften
...
\textit{dlsd}-Erfassungprozess diese mit dem Anfügen des nächsten
...
d. h. die Position des einleitenden \textit{\textless }-Zeichens.
...
\chapter{Der \textit{DLS Manager}}
\label{sec:manager}

Der \textit{DLS Manager} \index{DLS Manager@\textit{DLS Manager}} ist
die grafische Benutzeroberfläche zur Konfiguration der Messaufträge
\index{Messauftrag} in einem \textit{DLS}-Datenverzeichnis. Er dient weiterhin
zur Steuerung und Kontrolle der laufenden Erfassungsprozesse.

Wie auch der \textit{dlsd} kann diesem Tool in der Kommandozeile mit
dem Parameter \textit{-d} das \textit{DLS}-Daten\-verzeich\-nis
mitgeteilt werden, auf dem es arbeiten soll (siehe Anhang
\ref{sec:apx_cmd_dlsctl}).

Beim Programmstart führt der \textit{DLS Manager} selbstständig einige
Prüfungen durch:

\begin{itemize}
\item Wenn das angegebene \textit{DLS}-Datenverzeichnis ein noch leeres
  Verzeichnis ist, wird der Benutzer gefragt, ob darin eine gültige
  \textit{DLS}-Datenverzeichnis-Struktur angelegt werden soll.
\item Läuft zum angegebenen \textit{DLS}-Datenverzeichnis noch keine Instanz
  des \textit{dlsd}, wird der Benutzer gefragt, ob eine Instanz
...
\textit{DLS Managers}, welches direkt nach dem Starten des Programmes
sichtbar wird.
...
\begin{figure}[tbh]
  ...
  \end{center}
  \caption{Hauptdialog des \textit{DLS Managers}}
...
\item[Auftrags-ID und Bezeichnung] Die Beichnung des Auftrages ist
  ...
  \textit{DLS}-Datenverzeichnis im Unterverzeichnis
...
\item[Quelle] Die Adresse der Datenquelle. Dies kann ein Hostname oder
  ...
  sichergestellt sein, dass dieser vom \textit{dlsd} zur Laufzeit in
  eine IP-Adresse aufgelöst werden kann. Auf dem angegebenen Host muss
...
\item Betätigen der Schaltfläche \glqq OK\grqq\ (oder Drücken der
  ...
  geschlossen. Läuft ein \textit{dlsd} -Erfassungsprozess, wird dieser
...
\item Bei Betätigen der Schaltfläche \glqq OK\grqq\ werden alle
  ...
  \textit{dlsd}-Erfassungsprozess zur Übernahme der neuen
...
\item Betätigen der Schaltfläche \glqq OK\grqq\ prüft die angegebenen
  ...
  \textit{dlsd}-Erfassungsprozess zur Übernahme der neuen Parameter
...
\chapter{Der Betrachter \textit{DLS View}}
\label{sec:view}

\index{DLS View@\textit{DLS View}}
...
\begin{figure}[tbh]
  ...
  \end{center}
  \caption{Hauptdialog des Betrachters \textit{DLS View}}
...
\begin{figure}[tbh]
  ...
  \end{center}
  \caption{Export-Dialog des Betrachters \textit{DLS View}}
...
\item Bei Betätigen der Schaltfläche \glqq Exportieren\grqq\ beginnt
  ...
  \textit{\$DLS\_EXPORT}\index{\$DLS\_EXPORT@\textit{\$DLS\_EXPORT}}
  gesetzt ist, werden die Daten in das entsprechende Verzeichnis
  geschrieben. Andernfalls wird das aktuelle Verzeichnis
  genutzt. Damit beim Export keine Daten überschrieben werden, wird
  immer ein Unterverzeichnis erstellt, dass alle Datendateien
  enthält. Der Name dieses Unterverzeichnisses wird durch die
  Umgebungsvariable \textit{\$DLS\_EXPORT\_FMT}
  \index{\$DLS\_EXPORT\_FMT@\textit{\$DLS\_EXPORT\_FMT}} bestimmt, die
  Platzhalter nach den Konventionen der C-Funktion strftime()
  erlaubt. Siehe \textit{man 3 strftime} für eine Auflistung. Ist die
...
Das \textit{DLS}-System unterstützt verschiedene Kompressionsalgorithmen. Da
...
Kompressionsmethoden des \textit{DLS} das Suffix \textit{/Base64}.

Folgende Kompressionsmethoden werden vom \textit{DLS} unterstützt:
...
Kompressionsverfahren: \textit{ZLib/Base64}\\
Komprimierbare Datentypen: \textbf{Alle}.

Die Bibliothek \glqq ZLib\grqq\index{ZLib}
(\url{http://www.gzip.org/zlib}) stellt Funktionen zur
verlustfreien Kompression von Daten bereit. Der
\textit{dlsd}-Erfassungprozess nutzt diese im Kompressionsverfahren
\textit{ZLib/Base64}. Ausserdem wird der ZLib-Algorithmus zur
Unterstützung in den weiteren Verfahren genutzt, um die bereits
...
Kompressionsverfahren: \textit{MDCT/ZLib/Base64}\\
Komprimierbare Datentypen: \textit{TFLT}, \textit{TDBL}

Das Kompressionverfahren der \glqq modifizierten, diskreten
Cosinus-Transformation\grqq\ (MDCT) des \textit{DLS} ist eigentlich ein
...
Marios Athineos\footnote{marios@ee.columbia.edu,
  \url{http://www.ee.columbia.edu/\textasciitilde{}marios}, Columbia
  University} hat ein Verfahren entwickelt, um eine MDCT über $n$ Werte
  auf eine Fourier-Trans\-formation über $\frac{n}{4}$ Werte zu
  reduzieren. Der \textit{DLS} benutzt dieses Verfahren in Kombination
  mit der \textit{FFTW}-Bibliothek (siehe Anhang \ref{sec:apx_soft})
...
Kompressionsverfahren: \textit{Quant/ZLib/Base64}\\
Komprimierbare Datentypen: \textit{TFLT}, \textit{TDBL}
...
Der \textit{DLS} ist größtenteils in der Programmiersprache C++ \index{C++}
...
\item Für das Aufzeichnen der während der Laufzeit anfallenden Nachrichten
  wird der \textit{syslogd} \index{syslogd@\textit{syslogd}} verwendet,
  der standardmäßig jeder Linux-Distribution beiliegt.
\item Für das Kompilieren der grafischen Benutzeroberflächen
  \textit{DLS Manager} \index{DLS Manager@\textit{DLS Manager}} und
  \textit{DLS View} \index{DLS View@\textit{DLS View}} ist zusätzlich
  die GUI-Bibliothek \textit{FLTK} \index{FLTK@\textit{FLTK}} in der
  Version 1.1.\textit{X} nötig, die auf der \textit{FLTK}-Homepage
  \url{http://www.fltk.org} heruntergeladen werden kann.
...
\item Ebenfalls für die Kompression wird die \textit{FFTW}-Bibliothek
  benötigt. Mit dieser ist der \textit{DLS} in der Lage die für die
  ...
  \url{http://www.fftw.org/download.html} heruntergeladen werden.
\item Für den \textit{DLS Manager} und für \textit{FLTK} wird die
...
\begin{table}[htb]
  ...
  \begin{tabular}[thb]{|l|l|l|}
    ...
    \textit{TUCHAR} & 1 Byte Ganzzahl (ohne Vorzeichen) & ZLib/Base64
   \\
   ...
   \\
    ...
    \textit{TULINT} & 4 Byte Ganzzahl (ohne Vorzeichen) & ZLib/Base64
   \\
...
Das \textit{DLS}-System verwendet an mehreren Stellen
sog. \textit{PID}-Dateien, \index{PID-Dateien}
einen Mechanismus, der verhindern soll, dass für eine konkrete Aufgabe
mehrere Prozesse laufen. Eine \textit{PID}-Datei enthält die
\textit{ASCII}-kodierte Prozess-ID (\textit{PID}) des aktuell
...
\textit{PID}-Datei (d. h. der angegebene Prozess existiert nicht mehr)
...
\index{dlsd@\textit{dlsd}!Kommandozeilenparameter}
...
\index{dls\_status@\textit{dls\_status}!Kommandozeilenparameter}
...
\index{DLS Manager@\textit{DLS Manager}!Kommandozeilenparameter}
...
\index{DLS View@\textit{DLS View}!Kommandozeilenparameter}
...
\index{dls@\textit{dls} (Tool)!Kommandozeilenparameter}
...
\index{dls\_quota@\textit{dls\_quota}!Kommandozeilenparameter}
