...
void mdct(unsigned int exp2, const double *x, double *y)
{
  ...
  // FFTW berechnet unnormalisierte FFT. Zur Kompatiblität mit dem alten Verfahren so lassen!
...
void imdct(unsigned int exp2, const double *x, double *y)
{
  ...
  for (t = 0; t < m; t++)
  {
    rot[2 * t] = c_r[t] / two_n;    // " / two_n" hinzugefügt. Liefert selbe Werte, wie altes Verfahren. fp
