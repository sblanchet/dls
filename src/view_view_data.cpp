...
void ViewViewData::range_callback(void (*cb)(COMTime, COMTime, void *), void *data)
{
  _range_cb = cb;
  _range_cb_data = data;
}
...
void ViewViewData::draw()
{
  ...
  if (_channels.size()) _channel_height = _channel_area_height / _channels.size();
  ...
  // Zeitskala zeichnen
  _draw_time_scale(x(), y(), _channel_area_width, h() - INFOHOEHE - 2 * ABSTAND);
  ...
  while (channel_i != _channels.end())
  {
        ...
        && (channel_pos + 1) * _channel_height >= _scroll_pos)
    {
      ...
      if ((channel_pos + 1) * _channel_height - 1 - _scroll_pos >= _channel_area_height)
      {
        clip_height = _channel_area_height - channel_pos * _channel_height + _scroll_pos;
      }
                              ...
                              int width, int height)
{
  ...
  {
    xp = (chunk_range_i->start.to_dbl() - _range_start.to_dbl()) * _scale_x;
    ...
    old_xp = (chunk_range_i->end.to_dbl() - _range_start.to_dbl()) * _scale_x;
                                 ...
                                 int width, int height)
{
  ...
  if (channel->max() >= channel->min())
  {
    ...
    // Wenn die Werteskala nur einen Wert umfasst, Bereich zur Anzeige erweitern
    ...
    while (chunk_i != channel->chunks()->end())
    {
      ...
      {
        _draw_min_max(*chunk_i, left, drawing_top, width, drawing_height);
      }
  ...
  for (i = 0; i < chunk->gen_data()->size(); i++)
  {
    ...
    { 
      ...
      else
      {
        ...
        {
          // Linie zeichnen
          fl_line(left + old_xp, top + height - old_yp, left + xp, top + height - yp);
        }
      }
                              ...
                              int left, int top, int width, int height)
{
  ...
  // Auch wichtig, da fl_measure den Text sonst auf die Breite gebrochen berechnet
  text_time_width = 0;
  text_value_width = 0;
  ...
  if (_scan_found)
  {
    ...
    if (_scan_min != _scan_max)
    {
      ...
      if (yp2 != yp1)
      {
        fl_line(left, top + height - yp2, left + width, top + height - yp2);
      }
    }
  }
  ...
  fl_draw(text_time.str().c_str(), left + text_time_x, top + text_height / 2 + fl_descent());
  y_pos = text_height;

  if (_scan_found)
  {
    if (height - yp1 - 5 - text_height < y_pos) yp1 = height - y_pos - text_height - 5;

    // Text für Wert(e) zeichnen
    fl_color(FL_WHITE);
    fl_rectf(left + text_value_x - 2, top + height - yp1 - 5 - text_height, text_value_width + 4, text_height);
    fl_color(SCAN_COLOR);
    fl_draw(text_value.str().c_str(), left + text_value_x, top + height - yp1 - text_height / 2 - fl_descent());
  }
}
