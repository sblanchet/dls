...
template <class T> ViewChunkT<T>::ViewChunkT()
{
    _ring = 0;
    _compression = 0;
}
...
template <class T> inline const ViewData *ViewChunkT<T>::gen_data() const
{
    return &_gen_data;
}
...
template <class T> inline const ViewData *ViewChunkT<T>::min_data() const
{
    return &_min_data;
}
...
template <class T> inline const ViewData *ViewChunkT<T>::max_data() const
{
    return &_max_data;
}
...
template <class T> inline unsigned int ViewChunkT<T>::blocks_fetched() const
{
...
template <class T> bool ViewChunkT<T>::has_data() const
{
...
template <class T> void ViewChunkT<T>::fetch_data(COMTime start,
                                                  COMTime end,
...
template <class T> void ViewChunkT<T>::_load_data(ViewDataT<T> *data_list,
                                                  ...
                                                  COMTime start, COMTime end)
{
    ...
    for (i = 0; i < global_index.record_count(); i++) {
        ...
        data_file_name << "/data" << global_index_record.start_time
                       << "_" << meta_type;
            ...
            while (1) {
                ...
                if (xml.tag()->title() == "d") { // Ein Daten-Tag ist komplett
                    ...
                    if (data_list->size() == 0) {
                        ...
                        if (COMTime(index_record.start_time)
                            < data_list->start_time()) {
                            data_list->start_time(index_record.start_time);
                        }
                        if (COMTime(index_record.end_time)
                            > data_list->end_time()) {
                            data_list->end_time(index_record.end_time);
                        }
                    }
                    ...
                    if (data_list->start_time() < _start)
                        _start = data_list->start_time();
                    if (data_list->end_time() > _end)
                        _end = data_list->end_time();
