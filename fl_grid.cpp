...
int Fl_Grid::handle(int e)
{
  ...
  switch (e)
  {
    case FL_PUSH:
      ...
      else // Auf den Zeilenbereich geklickt
      {
        ...
        if (row_index >= 0
            ...
            && record_index < _record_count) // Auf einen Record geklickt
        {
          if (_select_mode != flgNoSelect)
          {
            ...
            else if (Fl::event_key(FL_Shift_L) || Fl::event_key(FL_Shift_R))
            {
          ...
          // Wenn kein Selektieren möglich, aber Checkboxes, dann führt
    ...
    case FL_KEYDOWN:
      ...
      {
        if (_scroll_index < _record_count - row_count) // Ist noch nicht am Ende
        {
